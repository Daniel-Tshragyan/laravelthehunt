...
@section('dialogs')
    <ul class="list-item">
        @foreach($parameters->dialogs as $dialog)
            <li><a href="
...
@endsection
@section('content1')
    @if(!is_null($parameters->user))
        <div class="col-12 col-lg-7 col-xl-9">
            <div class="py-2 px-4 border-bottom d-none d-lg-block">
                <div class="d-flex align-items-center py-1">
                    <div class="flex-grow-1 pl-3">
                        <strong>{{ $parameters->user->name }}</strong>
                    </div>
                </div>
            </div>
                        ...
                        >
                            ...
                            @if($message->file)
                                ...
                                @if($message->isFileImage())
                                    <a href="{{ route('download-file',['user' => $parameters->user->id,'message' => $message]) }}"><img
                                            src="{{ asset('storage/message_files/'.$message->file) }}" alt=""
                                            style="width:50px"></a>
                                @else
                                    <a href="{{ route('download-file',['user' => $parameters->user->id,'message' => $message]) }}">{{$message->file}}</a>
                                @endif
                            @endif
                            <br>
                            <span style="color:black">{{ $message->created_at }}</span>
                        </div>
                    @endforeach
                    @if($parameters->limit != 0)
                        <p style="text-align: center">
                            <a href="{{ route('open-message', ['user' => $parameters->user->id, 'limit' => $parameters->limit]) }}">
                                Load More
                            </a>
                        </p>
                    @endif
            ...
            <div class="flex-grow-0 py-3 px-4 border-top">
                <div class="input-group">
                    <form method="post" action="{{ route('send-message',['user' => $parameters->user]) }}"
